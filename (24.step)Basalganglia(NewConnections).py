...
###############################################################################
#Posterior Cortex NMDA neuron model
#Models value
N_NMDA = 80
C_NMDA = 50
vr_NMDA = -60#-55
...
def spikingprint(spk,clippeddata,N_group,freqgroup,tit=""):
    plot(spk.t/ms, spk.i, '.k',label = int(freqgroup)*Hz, ms = 8)
    xlim(0,runtime)
    ylabel('Neuron Cell',fontsize = 36,)
...
def spikingprint2(spk,clippeddata,N_group,freqgroup,tit=""):
    plot(spk.t/ms, spk.i, '.k',label = int(freqgroup)*Hz, ms = 2)
    xlim(0,runtime)
    ylabel('Neuron Cell',fontsize = 12)
...
def allprint():
    ...
    subplot(4,3,4); hist_MSND1 = firingbastır2(spk_MSND1,clippedMSND1,N_MSND1,freqMSND1,tit="D1 type optimal peak")
    subplot(4,3,5); hist_FSI = firingbastır2(spk_FSI,clippedFSI,N_FSI,freqFSI,tit="Fast Spiking Interneurons")
    subplot(4,3,6); hist_MSND2 = firingbastır2(spk_MSND2,clippedMSND2,N_MSND2,freqMSND2,tit="D2 type optimal peak")
    subplot(4,3,7); hist_GPi = firingbastır2(spk_GPi,clippedGPi,N_GPi,freqGPi,tit="Globus Pallidus inside")
    subplot(4,3,8); hist_STN = firingbastır2(spk_STN,clippedSTN,N_STN,freqSTN,tit="Subthalamic Cell")
...
def differprint():
    ...
    figure(figsize=(18,6)); hist_MSND1 = firingbastır(spk_MSND1,clippedMSND1,N_MSND1,freqMSND1,tit="D1 type optimal peak")
    figure(figsize=(18,6)); hist_FSI = firingbastır(spk_FSI,clippedFSI,N_FSI,freqFSI,tit="Fast Spiking Interneurons")
    figure(figsize=(18,6)); hist_MSND2 = firingbastır(spk_MSND2,clippedMSND2,N_MSND2,freqMSND2,tit="D2 type optimal peak")
    figure(figsize=(18,6)); hist_GPi = firingbastır(spk_GPi,clippedGPi,N_GPi,freqGPi,tit="Globus Pallidus inside")
    figure(figsize=(18,6)); hist_STN = firingbastır(spk_STN,clippedSTN,N_STN,freqSTN,tit="Subthalamic Cell")
